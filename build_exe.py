...
def version_resource(target: Path, version: str) -> Path:
    """Write the Windows VERSIONINFO resource PyInstaller stamps into the exe.

    WHY A BINARY WITH NO VERSION INFO IS A PROBLEM AND NOT AN OMISSION. Defender's ML models
    weigh how much an executable says about itself, and 0.9.2 was flagged
    `Trojan:Win32/Wacatac.C!ml` — a generic verdict, not a signature match. An unsigned,
    freshly built one-file bundle with near-zero prevalence starts out suspicious; one that
    also declines to name its publisher, product or version is a blank to every tool that
    looks at it, including the Properties dialog a wary player opens before running it.

    It is not a fix — the honest fix is code signing, and the remedy for a specific detection
    is a developer submission to Microsoft. This removes one weight, costs nothing, and gives
    a player who right-clicks the file something to read.

    GENERATED FROM `CLIENT_VERSION` rather than written down, for the reason `_client_version`
    exists at all: a fourth copy of the number would agree with the other three until it did
    not, and the one place it would surface is a Properties dialog nobody checks.
    """
    parts = [int(n) for n in re.findall(r"\d+", version)][:4]
    parts += [0] * (4 - len(parts))
    quad = ", ".join(str(n) for n in parts)
    # 040904B0: US English, Unicode codepage — and the Translation below must agree with it,
    # or Windows shows the strings for a language it then cannot find.
    body = f"""# Generated by build_exe.version_resource — do not edit.
VSVersionInfo(
  ffi=FixedFileInfo(
    filevers=({quad}),
    prodvers=({quad}),
  ...
  kids=[
    StringFileInfo([
      StringTable(
        '040904B0',
        [StringStruct('CompanyName', 'exkuretrol'),
         StringStruct('FileDescription', 'wddrop - reads the drop messages Wizardry '
                                         'Variants Daphne prints on your own screen'),
         StringStruct('FileVersion', '{version}'),
         StringStruct('InternalName', 'wddrop'),
         StringStruct('LegalCopyright', 'Copyright 2026 exkuretrol. Apache-2.0. '
                                        'Not affiliated with Drecom.'),
         StringStruct('OriginalFilename', 'wddrop.exe'),
         StringStruct('ProductName', 'wddrop'),
...
def main(argv: list[str] | None = None) -> int:
    ...
    cmd = [
        ...
        # EXPLICIT, because the default is "use UPX if it happens to be on PATH" — so
        # whether the exe ships packed depended on the runner's image rather than on any
        # decision here, and a packed binary is one more thing Defender's ML weighs against
        # an unsigned bundle. Saying it costs a few MB of download and removes a variable
        # from a build whose output is already being called a trojan.
        "--noupx",
        "--name", args.name,
    ...
    # WINDOWS ONLY: the resource is a PE structure, and PyInstaller refuses the flag on any
    # other platform rather than ignoring it — which would turn a Linux smoke build of this
    # script into a failure that has nothing to do with what it was testing.
    if _windows():
        cmd += ["--version-file",
                str(version_resource(build / f"{args.name}_version.txt", _client_version()))]
    if not args.console:
        cmd.append("--windowed")
